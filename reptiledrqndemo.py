...
task_sets_config = {
    ...
    "test": {"num_targets": 4, "num_agents": 1, "density": 0.4, "width": 16, "height": 16, "obs_radius": 5},
}
...
class TestAgent(DRQNAgent):
    ...
    # 这个方法用于在特定任务上进行微调
    def fine_tune_on_task(self, task_env, num_episodes=10, num_steps_per_episode=16):
        print(f"开始在新任务上微调 {num_episodes} 个回合...")
        
        # 为微调重置/设置 epsilon
        self.epsilon = 0.5 # 开始时进行一些探索
        self.epsilon_min = 0.1
        ...
        for ep in range(num_episodes):
            ...
            print(f"  微调回合 {ep+1}/{num_episodes} 完成 (长度: {ep_len})。Buffer size: {len(self.replay_buffer)}")

            # --- 2. 训练 (调用继承的 train) ---
            if len(self.replay_buffer) >= self.batch_size:
                # 运行几步梯度下降
                for _ in range(num_steps_per_episode):
                    self.train() # <--- 成功复用 train() 方法
                    total_steps_trained += 1
        
        print(f"微调完成。共训练 {total_steps_trained} 步。")
...
if __name__ == '__main__':
    ...
    env = test_agent.task_env(task_sets_config['test'])
