    ...
    def evaluate(self, task_env, render_animation=False, animation_filename="task_evaluation.svg"):
        ...
        with torch.no_grad():
            while not (all(terminated) or all(truncated)):
                ...
                actions_np, new_hidden_state = self.select_actions(states_tensor, current_hidden_state)
                current_hidden_state = new_hidden_state
                print(f"步骤 {step_count}: 选择动作 {actions_np}")
...
if __name__ == '__main__':
    ...
    try:
        ...
        print("训练中断，保存当前模型权重...")
        torch.save(reptile.meta_agent.q_net.state_dict(), 'reptile_drqn_meta_agent_interrupt.pth')
    torch.save(reptile.meta_agent.q_net.state_dict(), 'reptile_drqn_meta_agent.pth')
    # 测试训练好的模型
    test_agent = TestAgent(
        state_shape=(3, 11, 11),
        num_actions=5,
        state_dict_file_path='reptile_drqn_meta_agent.pth',
        finetune_lr=0.00001,
    ...
    test_agent.fine_tune_on_task(env, num_episodes=20, num_steps_per_episode=32)                # 微调
