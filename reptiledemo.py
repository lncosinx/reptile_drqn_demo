...
# 定义DRQN智能体
class DRQNAgent:
    def __init__(self, num_agents, state_shape, num_actions, replay_buffer, lr, model_path=None):
        self.num_agents = num_agents
        self.state_shape = state_shape
        self.num_actions = num_actions
        self.lr = lr
        self.q_net = CnnQnet(state_shape, num_actions).to(device)
        self.q_net.load_state_dict(torch.load(model_path, map_location=device)) if model_path else None
...
class Reptile:
    def __init__(self, all_task_sets, state_shape, num_actions, num_agents, task_sets, model_path=None):
        ...
        # 2. 创建元智能体 (meta-agent)
        # 它持有“元权重” (phi)
        self.meta_agent = DRQNAgent(num_agents, state_shape, num_actions, template_buffer, lr=self.inner_lr, model_path=model_path)
    ...
    def meta_train(self):
        ...
        for meta_iter in range(self.meta_iterations):
            ...
            if (meta_iter + 1) % self.save_freq == 0:
                 try:
                    # 修正：保存 meta_agent 的状态字典
                    torch.save(self.meta_agent.q_net.state_dict(), self.model_save_path_interrupt)
                    print(f"--- Meta Iter {meta_iter + 1}: 中断模型已保存到 {self.model_save_path_interrupt} ---")
        ...
        # 训练结束后保存最终模型
        try:
            torch.save(self.meta_agent.q_net.state_dict(), self.model_save_path_final)
            print(f"训练完成。最终模型已保存到 {self.model_save_path_final}")
        except Exception as e:
            print(f"恭喜，又浪费了一天：{e}")
...
if __name__ == '__main__':
    ...
    test_agent = TestAgent(
        state_shape=(3, 11, 11),
        num_actions=5,
        state_dict_file_path='reptile_drqn.pth',
        finetune_lr=0.00001,
        finetune_buffer_capacity=500,
        seq_len=8,
        num_agents_to_test=1
    )
    env = test_agent.task_env(task_sets_config['T5'])
